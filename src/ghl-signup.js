...
async function handleGHLSignup(req, res) {
  try {
    ...
    // ✅ UPDATED: Now returns object with {knowledgeBaseId, fileId}
    // ============================================
    ...
    if (websiteUrl && websiteUrl.trim().length > 0) {
      ...
      try {
        ...
        if (knowledgeBaseData) {
          ...
          console.log(`✅ File ID: ${knowledgeBaseData.fileId}`);
        } else {
    ...
    // ✅ UPDATED: Now passes knowledgeBaseData object (includes fileId)
    // ============================================
    ...
    const assistant = await createIndustryAssistant(
      businessName,
      industry,
      knowledgeBaseData,  // ✅ CHANGED: Pass full object instead of just ID
      formattedOwnerPhone,
    ...
    const { data: newClient, error: clientError } = await supabase
      ...
      .insert({
        ...
        knowledge_base_id: knowledgeBaseData?.knowledgeBaseId || null,  // ✅ CHANGED: Extract from object
